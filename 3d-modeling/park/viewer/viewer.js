...
async function select(id,{focusScene=false}={}){const token=++request;const v=nav.views.find(x=>x.id===id);if(!v)return;clearInput();station=null;busy=true;updateLadder();status('会場を読み込み中…');$('retry').hidden=true;try{await load('park');await load('rest');await load('management');if(token!==request)return;selected=v;fillViews(areaOf(v));$('view').value=id;camera.position.copy(converted(v.position));lookAt(v.target);startOrbit(v);updateControls();dirty=true;$('place').textContent=orbit?'WASD／矢印でも中心を移動。「全体」で位置・角度・距離を戻せます。':v.fixed?'鑑賞視点：見回しできます。移動は園路や室内の場所を選んでください。':v.label;status('');if(v.id==='rest_barrel_peek')beginLadder();
