<!-- ENTRANCE56 CURRENT -->
休憩棟の東側玄関と《星くず》を修正した確認用案です（PLAN §56、未公開）。玄関は東端のままで、引き戸の面だけ5cm奥へ納め、低い下屋・庇・樋と両脇の外面を調整しました。受付・樽の間とその奥の部屋は保持。右側は写真に写る外壁と格子だけで、**その奥の構造は未確認です**。

《星くず》A02は、制作者の訂正に従い、玄関を入って左の板廊下の奥の角へ移しました。旧位置からは撤去し、写真のアンケート用の低い机・白箱を概形で添えました。ミニマップの②も移設先を示します。他の作品・管理棟は保持しています。

[公園ビューア](park/viewer/index.html)／[玄関とA02の平面図](out/entrance56/entry_plan.jpg)／[休憩棟配置図](out/rest_layout.jpg)／[ブラウザ確認手順](out/entrance56/BROWSER_CHECK.md)／[今回だけの置換表](out/entrance56/publish_manifest.json)。確認対象は「東側の正式な玄関」「東側玄関のポスター」「入口・板の廊下」と、公園の「休憩棟の玄関」です。板廊下の視点から玄関側の床へ見下ろすと②が見えます。

写真6412の左の長い翼と大屋根の見え方には未一致が残ります。部屋を移す解や、未確認の床を伴う大きな張り出しは採用していません。実測・図面がないため寸法は推定です。§54は中断のまま。**公開はせず、Claudeが平面図と比較を制作者に示して了承後に差し替えるための成果物です。** 以下の旧版の「最新」「公開待ち」は当時の記録です。
<!-- END ENTRANCE56 CURRENT -->
...
作品は `Artworks`、展示台・椅子・キャプション等は `Exhibit_Furniture` にあります。`A01`〜`A24` は英司のリスト、`S01`〜`S16` は英治のリストで、発行版の番号に統一しました。各番号のEmptyを親として、その作品のパーツと専用什器をまとめて移動できます。現行配置の正本は `placements.json`。親の座標・`position_note`・`placement_confidence` と照合してください。更新対象の `room_id` は現在の部屋で、生成当初の `room`・`position` 属性が残る場合は正本を優先します。
...
## §57 東側玄関と受付の机（確認用・未公開）
玄関の引き戸は東端X17.10、開口中心Y7.43へ固定しています。右の格子窓の壁を直角に東へ2.40m張り出す外殻にし、低い下屋・主屋根の軒・石段・案内板を調整しました。右側の内部は未確認で、部屋や通路は作っていません。左のガラス全幅は写真と未一致です。
受付机は写真7928（撮影：青木兼治）の木製天板と脚、紙類を形と色で再現。0.60×1.05×高さ0.75m、受付RE内の位置は推定です。椅子など写真で確定できない物は足していません。実寸の保証ではありません。《星くず》の位置(16.70,5.51)は制作者確認済みで固定しました。
[平面図](out/entrance57/entry_plan.jpg)と検証用private比較を制作者に示してから公開します。ブラウザでは「東側の正式な玄関」「東側玄関のポスター」「入口・板の廊下」を確認してください。[確認手順](out/entrance57/BROWSER_CHECK.md)、§56＋57をまとめた置換表はout/entrance57/publish_manifest.json。§54の未着手分は再開していません。

### §58 記録：東側玄関と樽の裏の通路（受付と2作品は§59で更新）
制作者の承認により、引き戸は東面X17.10のままY7.43から10.68へ北に3.25m移動。樽の間の北に主区間で有効約1.28mの通路を設け（西端の既存区画沿いの通行幅は約0.88m）、既存中央通路と、開けた北襖から樽の間・エデンの間へつないだ。西端の実際の接続は写真で未確認。室の位置・寸法、樽と梯子、星くず、A01/A09以外の作品は固定した。
受付は入口の左（南）のガラス内側。机は写真6412・7928を参照した1.05×0.60×高さ0.75mの仮寸、椅子や人物は追加していない。A01/A09は向かいの樽の間の東壁沿い、正確な座標は推定。右の張り出しは外面だけで内部未確認。新しい画像テクスチャは使っていない。
視点「玄関・樽の裏の通路」（旧「入口・板の廊下」）で玄関から西への通路を確認できる。ミニマップと公園の玄関接続も追従。写真一致の数値はPLAN58とout/entrance58/photo_residuals.jsonを参照。写真からの条件付き推定であり、実測寸法や全体の完全一致を保証しない。
§58時点の比較はout/entrance58/に保存。現在の確認案・置換表は下の§59を使用する。private/は記録写真を含み公開コピーしない。ブラウザ実描画はClaudeによる確認待ち。

## §59 受付と作品配置（未公開）
受付奥を開いた障子にし、開口に机、その奥に既存の樽が見える配置へ修正。玄関・北通路と各室・星くずは不変。COMEは連結広縁の庭側、トルソーは星の間西壁へ暫定移設（実展示位置は未確認）。写真7928は東広縁と暫定判定し、机/欄間の寸法・割付は写真からの推定。実測・図面は無い。
Claudeのブラウザ確認：公園ビューアをfile://とHTTPで開き、「休憩棟」→「東側の正式な玄関」「玄関・樽の裏の通路」「樽の間」「星の間」「エデンの間」。ガラス越しの机と樽、開口脇の通過、梯子、ミニマップの①⑨、スマホ390pxの操作を確認。棟別も同じ順で確認する。描画/画像は更新済みだが実ブラウザ未確認。
平面図out/entrance59/entry_plan.jpg、写真比較out/entrance59/private/、検査out/entrance59/verification.json、描画時間out/entrance59/render_timing.json。§59当時の置換表out/entrance59/publish_manifest.jsonは§56〜59合併（現行は下の§60）、公開済み55との差分。比較と平面図の制作者確認前に公開しない。

## §60 北通路の襖（未公開）
樽の間・エデンの間の北側襖は各4枚すべて閉じています。北通路から直接は入れません。樽の間へは受付の開いた障子、エデンの間は西端を回った既存の広縁・板廊下側の鑑賞位置へ進めます（床作品を保護するため室内中央は従来から歩行不可）。全17視点と梯子への到達を検査済み。玄関・各室・作品・受付は§59から不変です。
Claude確認：休憩棟の「玄関・樽の裏の通路」「樽の間」「エデンの間」「梯子から樽の中」。北側襖を通り抜けないこと、受付開口と西端経路、地図の閉鎖線をfile://とHTTP、390pxで確認。実ブラウザは未確認です。
確認画像out/fusuma60/comparisons.jpg、検査out/fusuma60/verification.json。現在の置換表はout/fusuma60/publish_manifest.json（§56〜60合併、公開済み55との差分）。比較を制作者へ示して了承を得るまで公開しない。

## §61 COME（仮）の訂正（未公開）
4本を鉛直のC・O・M・E断面に直し、板の間の東の塗り壁沿いへ移しました。高さ1.30m、断面・台寸法、正確な展示位置は推定です。玄関・他の作品・北通路の閉じた襖は保持。「板の間」の視点で確認できます。旧広縁の位置からは撤去済みです。
Claude確認：公園/棟別ビューアの「板の間」、旧位置の「エデンの間」、ミニマップ①、板の間への出入り。file://・HTTP・390pxで確認してください。ブラウザの実描画は未確認。private/の比較画像は公開対象外。置換表out/come61/publish_manifest.jsonは§60からの今回の差分のみで、制作者確認まで公開しません。

## §62 星くずの内側（未公開）
「休憩棟→星くずを覗く」で、黒い袋の内側と底に散らばる1,500粒の白〜淡金色の光点を見られます。作品の位置・外形は固定。粒の実体や細かな形は写真からの近似です。Blenderでは反射と弱い発光、ビューアでは明るい色と既存の発光対応を使用し、袋の物理的な透過と網目の影は省略しています。写真画像は貼っていません。
Claude確認：file://・HTTP・390pxで「星くずを覗く」、ミニマップ②付近からの移動、受付と梯子、§61の板の間のCOME。ブラウザ実描画は未確認。置換表out/stardust62/publish_manifest.jsonは§61＋§62（公開済み60との差分）。private比較画像は公開対象外。制作者確認まで公開しません。

## §65 人工芝の間（未公開・写真一致は未達）
赤い寝椅子は入口正面の西窓、戸棚/A10は左の南壁、A08の文字ボード4枚と机は入口の外の板廊下です。§63の隣室と北への拡張を撤回し、入口と部屋の位置を保持しました。A01は人工芝の間の北側にある板床の塗り壁沿いへ仮配置。作品の転写画像は保持し、額や肖像は色面です。
「人工芝の間・窓と寝椅子」「人工芝の間から入口・廊下」「人工芝の間・飾り戸棚」「板廊下・問いの部屋」で確認できます。寸法と所在は写真からの推定です。白襖と額は写真の折れから北壁の折返しに置きました。写真の確認点1%目標は未達で、厳密な再現の完成版ではありません。技術検査は別に記録し、公開していません。平面はout/turf65/entry_plan.jpg、比較はprivate/、置換表はout/turf65/publish_manifest.json。

## §66 人工芝の間と廊下（未公開候補）
A08の4枚の背後を連続する板壁にし、エデンへの見通しを塞ぎました。人工芝の間は写真から東西3.32×南北4.68mと推定、寝椅子を長さ1.16mへ縮小。戸棚・上下2枚の説明パネル・窓を隣り合わせにしました。エデンと樽の間、玄関の位置、受付、星くずは固定し、玄関から右へ折れる北通路に変更しています。北側外形/屋根とA01の壁際を追従させました。
実測値ではなく、北通路西端と人工芝の間の西の外側帯は未確認。写真確認点の中央値は6167が1.51%、6168が0.87%、6216が0.76%。6167は1%目標未達で、写真の完全な再現とはしていません。技術検査は別に記録。§65の未公開候補を含めてout/turf66/publish_manifest.jsonにまとめ、公開は制作者確認後にClaudeが行います。操作確認は人工芝の4視点、A08の背後、右へ折れる北通路とエデン南側出入口で。

## §67 南廊下の表示復元（未公開）
星くずから樽の間・エデンの間の南を通って星の間へ続く廊下は、保存モデルでは§62のまま残っていました。§66の説明図で欠けた帯を復元し、ミニマップにも幅1.20mの南廊下と幅1.35mの西側広縁を明示しました。南廊下だけで各室へ歩けることを検査しています。採用された人工芝の間・玄関・全作品は固定。北通路と4枚パネルの視点を引き直し、北通路に推定の和風灯具1灯を追加しました。§65〜67の置換表はout/turf67/publish_manifest.json。公開前にClaudeが比較と歩行を確認します。
