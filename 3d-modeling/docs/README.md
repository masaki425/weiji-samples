...
一つのファイルだけで渡す場合は [viewer/展示室を歩く.html](viewer/展示室を歩く.html) を使えます。約130.2MBで、両棟のデータを含む単体版です。旧 `index.html` のローカル読込で使っていたfetchを廃止し、どちらの入口も同じ読込処理を使います。更新時は古い版にファイルを混ぜず、ZIPを新しいフォルダへすべて展開してください。
...
出典：**月の錯視の写真：撮影者未確認／ログビネンコ錯視：A. Logvinenko の図（展示写真からの転写）／記録写真：青木兼治**。画像使用は今回の指示に基づく例外5・6です。元画像の撮影者と権利は確認待ちです。管理棟の許可画像は8枚、休憩棟は1枚です。
